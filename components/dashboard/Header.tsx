...
export function Header({ onOpenAddModal, onOpenUploadModal, onRefreshData }: HeaderProps) {
  ...
  const handleRunClaudeLLM = async () => {
    setIsClassifying(true);
    setToastMsg("Collapsing counterparties & classifying via Claude 3.5...");
    try {
      const res = await fetch("/api/transactions/classify-llm", { method: "POST" });
      const data = await res.json();
      if (data.success) {
        setToastMsg(`✅ Classified ${data.metrics.updatedCount} transactions across ${data.metrics.distinctCounterparties} counterparties in ${data.metrics.apiCallsMade} parallel Claude API call(s) (${data.metrics.reductionPercentage}% token reduction)!`);
        if (onRefreshData) onRefreshData();
        setTimeout(() => setToastMsg(null), 6000);
      } else {
        setToastMsg(`❌ ${data.error || "Classification failed"}`);
        setTimeout(() => setToastMsg(null), 5000);
      }
    } catch (e: any) {
      console.error(e);
      setToastMsg("❌ Failed to reach Claude LLM classification endpoint.");
      setTimeout(() => setToastMsg(null), 5000);
    } finally {
    ...
    <header className="bg-white border-b border-slate-200/80 sticky top-0 z-20">
      <div className="px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2 sm:space-x-3">
          <button
            onClick={handleRunClaudeLLM}
            disabled={isClassifying}
            title="Batch classify distinct counterparties via Claude 3.5 LLM with token reduction"
            className="inline-flex items-center text-xs font-semibold px-2.5 sm:px-3 py-1.5 rounded-lg border border-purple-200 bg-purple-50 text-purple-800 hover:bg-purple-100 hover:border-purple-300 transition disabled:opacity-50 shadow-sm"
          >
            <Sparkles className={`w-3.5 h-3.5 mr-1.5 text-purple-700 ${isClassifying ? "animate-spin" : ""}`} />
            <span>{isClassifying ? "Classifying..." : "Run Claude LLM"}</span>
          </button>
          ...
          >
            <RefreshCw className={`w-3.5 h-3.5 mr-1.5 text-slate-500 ${isResetting ? "animate-spin" : ""}`} />
      ...
      {toastMsg && (
        <div className="bg-purple-900 text-white text-xs px-4 py-2 text-center font-medium flex items-center justify-center space-x-2 animate-in fade-in">
          <span>{toastMsg}</span>
          <button
            onClick={() => setToastMsg(null)}
            className="ml-3 text-purple-300 hover:text-white underline text-[11px]"
          >
            Dismiss
          </button>
        </div>
